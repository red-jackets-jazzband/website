...
/*
  Icon-only external links in the action cluster (#sheetActions, alongside
  Print/Export MP3/iReal Pro) for a tune's non-YouTube F: field references —
  Spotify, SoundCloud, SecondHandSongs, or anything else a URL classifies as.
  Each just opens the target site in a new tab; only a YouTube reference gets
  the docked LoopTube player in inspiration.js, since that's the one platform
  with an embeddable IFrame API this project drives directly. Rebuilt on
  every render from the current tune's `links` (already parsed by
  sheet.js's engrave() via lib/inspiration-links.js), same as
  irealpro-link.js's updateIrealProLink.
*/
export function updateInspirationExtLinks(links) {
  const actions = byId("sheetActions");
  if (!actions) return;
  actions.querySelectorAll(`[id^="${BUTTON_ID_PREFIX}"]`).forEach((node) => node.remove());
